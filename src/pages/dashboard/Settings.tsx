...
import { Building, Globe, Phone, MapPin, Upload, Plus, ChevronDown, ChevronRight, Shield, HelpCircle, FileText } from "lucide-react";
...
const Settings = () => {
  ...
  const [newDept, setNewDept] = useState({ degree: "", name: "" });
  ...
  const handleAddDegree = () => {
    if (newDegree.trim()) {
      setDegrees([...degrees, { name: newDegree, departments: [] }]);
      setNewDegree("");
  ...
  const handleAddDepartment = () => {
    if (newDept.degree && newDept.name.trim()) {
      setDegrees(degrees.map(d => 
        d.name === newDept.degree 
          ? { ...d, departments: [...d.departments, newDept.name] }
          : d
      ));
      setNewDept({ degree: "", name: "" });
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 border-2 border-primary/20">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <Button onClick={() => setShowAddDegreeModal(true)} variant="outline" size="sm" className="border-primary text-primary hover:bg-primary hover:text-white">
              <Plus className="w-4 h-4 mr-1" />
              Add Degree
            </Button>
            <Button onClick={() => setShowAddDeptModal(true)} variant="outline" size="sm" className="border-primary text-primary hover:bg-primary hover:text-white">
              <Plus className="w-4 h-4 mr-1" />
              Add Department
            </Button>
          </div>
        ...
        <div className="space-y-3">
          {degrees.map((degree) => (
            <div key={degree.name} className="border-2 border-primary/20 rounded-lg overflow-hidden">
              <button
      ...
      <Card className="p-6 border-2 border-primary/20">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 border-2 border-primary/20 rounded-lg hover:bg-muted/50 transition-all">
            <div>
              <h3 className="font-semibold">Change Password</h3>
              <p className="text-sm text-muted-foreground">Update your account password</p>
            </div>
            <Button variant="outline" size="sm" className="border-primary text-primary hover:bg-primary hover:text-white">Manage</Button>
          </div>
          <div className="flex items-center justify-between p-4 border-2 border-primary/20 rounded-lg hover:bg-muted/50 transition-all">
            <div>
            ...
            <Button variant="outline" size="sm" className="border-primary text-primary hover:bg-primary hover:text-white">Configure</Button>
          </div>
          <div className="flex items-center justify-between p-4 border-2 border-primary/20 rounded-lg hover:bg-muted/50 transition-all">
            <div>
            ...
            <Button variant="outline" size="sm" className="border-primary text-primary hover:bg-primary hover:text-white">View</Button>
          </div>
        </div>
      </Card>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 border-2 border-primary/20 rounded-lg hover:bg-muted/50 transition-all">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Access platform guides</p>
            </div>
            <Button variant="outline" size="sm" className="border-primary text-primary hover:bg-primary hover:text-white">View</Button>
          </div>
          <div className="flex items-center justify-between p-4 border-2 border-primary/20 rounded-lg hover:bg-muted/50 transition-all">
            ...
            <Button variant="outline" size="sm" className="border-primary text-primary hover:bg-primary hover:text-white">Contact</Button>
          </div>
        </div>
      </Card>
      ...
      {/* Add Degree Modal */}
      <Dialog open={showAddDegreeModal} onOpenChange={setShowAddDegreeModal}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="new-degree">Degree Name</Label>
              <Input 
                id="new-degree" 
            ...
            <div className="flex gap-3">
              <Button onClick={handleAddDegree} className="flex-1 bg-primary hover:bg-primary/90">
                Add Degree
              </Button>
              <Button variant="outline" onClick={() => setShowAddDegreeModal(false)}>
                Cancel
              </Button>
            </div>
          </div>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <select
                ...
                value={newDept.degree}
                onChange={(e) => setNewDept({...newDept, degree: e.target.value})}
              >
            ...
            <div className="flex gap-3">
              <Button onClick={handleAddDepartment} className="flex-1 bg-primary hover:bg-primary/90">
                Add Department
              </Button>
